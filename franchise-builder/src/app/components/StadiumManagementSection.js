...
import { STADIUM_TIERS, STADIUM_TIER_ORDER, getStadiumTierFromCapacity } from '@/data/leagues';
import {
  applyStadiumUpgrade,
  startStadiumProject,
...
const PROJECT_TARGETS = ['mid', 'large', 'mega'];
const TARGET_BASE_COSTS = { mid: 120, large: 200, mega: 280 };
...
const PREMIUM_SEATING_OPTIONS = [
  {
    key: 'luxury_box',
    title: 'Luxury Boxes',
    description: '$2M install · $0.8M/yr revenue each',
    countLabel: 'Owned',
    countField: 'luxuryBoxes',
    cost: 2,
    buttonLabel: 'Buy Box',
  },
  {
    key: 'club_section',
    title: 'Club Seat Sections',
    description: '$0.5M/section · $0.15M/yr (win-adjusted)',
    countLabel: 'Sections',
    countField: 'clubSeatSections',
    cost: 0.5,
    buttonLabel: 'Buy Section',
  },
];

export default function StadiumManagementSection({ fr, setFr, season }) {
  ...
  function handleStartProject() {
    ...
    alert('Cannot afford down payment for this project.');
  }

  function handlePremiumSeatingPurchase(type, cost) {
    if (!canAfford(fr.cash, cost)) return;
    const result = purchasePremiumSeating(fr, type, 1);
    ...
    <>
      ...
      <div className="card" style={{ padding: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {PREMIUM_SEATING_OPTIONS.map(({ key, title, description, countLabel, countField, cost, buttonLabel }) => {
            const affordable = canAfford(fr.cash, cost);
            return (
              <div key={key} className="card" style={{ padding: '10px 14px', flex: 1, minWidth: 140 }}>
                <div className="font-body" style={{ fontSize: '0.8rem', fontWeight: 600 }}>{title}</div>
                ...
                <div style={{ marginTop: 6 }}>
                  ...
                  <span className="font-mono" style={{ fontSize: '0.85rem' }}>{fr[countField] || 0}</span>
                </div>
                <button
                  className="btn-secondary"
                  style={{ fontSize: '0.65rem', marginTop: 8, opacity: affordable ? 1 : 0.4 }}
                  disabled={!affordable}
                  onClick={() => handlePremiumSeatingPurchase(key, cost)}
                >
                  {buttonLabel} (${cost}M)
                </button>
              </div>
          ...
          <div className="card-elevated" style={{ maxWidth: 400, width: '100%', padding: 20 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div className="font-body" style={{ fontSize: '0.7rem', color: 'var(--ink-muted)', marginTop: 6 }}>
                Base cost: ${TARGET_BASE_COSTS[projectTarget]}M · {projectTarget === 'mega' ? '3 seasons' : '2 seasons'}
              </div>
            </div>
