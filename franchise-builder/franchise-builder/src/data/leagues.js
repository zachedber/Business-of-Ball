...
// --- MARKET TIERS ---
// Tier 1 (Elite): 85+ market | Tier 2 (Major): 72-84 | Tier 3 (Mid): 62-71 | Tier 4 (Small): 55-61 | Tier 5 (Budget): <55
export const MARKET_TIERS = {
  1: { label: 'Elite', min: 85, color: '#D4A843', desc: 'Top media markets, massive revenue potential' },
  2: { label: 'Major', min: 72, color: '#2A5FA0', desc: 'Large markets with strong fan bases' },
  3: { label: 'Mid-Market', min: 62, color: '#1A6B3A', desc: 'Competitive cities with solid support' },
  4: { label: 'Small', min: 55, color: '#C47B18', desc: 'Smaller markets — build from the ground up' },
  5: { label: 'Budget', min: 0, color: '#C8202A', desc: 'Underdog markets — cheapest entry, highest challenge' },
};

export function getMarketTier(market) {
  if (market >= 85) return 1;
  if (market >= 72) return 2;
  if (market >= 62) return 3;
  if (market >= 55) return 4;
  return 5;
}

export function getMarketTierInfo(market) {
  return MARKET_TIERS[getMarketTier(market)];
}
