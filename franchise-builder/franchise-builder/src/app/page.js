...
import {
  ...
  generateStakeOffers, calculateStakeIncome,
  generateRivalryEvent,
} from '@/lib/engine';
...
function SetupScreen({ onCreateFranchise }) {
  ...
  // Only allow tiers 3-5 for starting franchise
  const allowedTiers = [3, 4, 5];
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px' }}>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
        <button
          ...
          onClick={() => { setSelectedLeague('ngl'); setSelectedTeam(null); setSelectedTier(null); }}
        ...
        <button
          ...
          onClick={() => { setSelectedLeague('abl'); setSelectedTeam(null); setSelectedTier(null); }}
          style={{ flex: 1, padding: '16px 20px' }}
        >
      ...
      {selectedLeague && (
        <>
          ...
          <p className="font-body" style={{ fontSize: '0.85rem', color: 'var(--ink-muted)', marginBottom: 16 }}>
            New GMs start with mid-market, small, or budget franchises. Earn your way to the elite markets.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
            {Object.entries(MARKET_TIERS).map(([tier, info]) => {
              const t = Number(tier);
              const locked = !allowedTiers.includes(t);
              const count = tierGroups[t]?.length || 0;
              return (
                <button
                  key={tier}
                  onClick={() => { if (!locked) { setSelectedTier(t); setSelectedTeam(null); }}}
                  disabled={locked}
                  className="card"
                  style={{
                    padding: '12px 16px', cursor: locked ? 'not-allowed' : 'pointer',
                    textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    border: selectedTier === t ? `2px solid ${info.color}` : '1px solid var(--cream-darker)',
                    opacity: locked ? 0.45 : 1,
                    background: selectedTier === t ? 'var(--cream-dark)' : 'var(--cream)',
                  }}
                >
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span className="font-display" style={{ fontSize: '0.95rem', fontWeight: 700, color: info.color }}>
                        TIER {tier} — {info.label}
                      </span>
                      {locked && <span className="badge badge-ink" style={{ fontSize: '0.6rem' }}>LOCKED</span>}
                    </div>
                    <div className="font-body" style={{ fontSize: '0.75rem', color: 'var(--ink-muted)', marginTop: 2 }}>
                      {info.desc}
                    </div>
                  </div>
                  <span className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--ink-muted)' }}>
                    {count} teams
                  </span>
                </button>
              );
          ...
          {selectedTier && tierGroups[selectedTier] && (
            <>
              <h3 className="font-display section-header" style={{ fontSize: '1.1rem' }}>
                Pick Your Franchise
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8, marginBottom: 24 }}>
                {tierGroups[selectedTier].map(t => (
                  <button
                    key={t.id}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <span className="font-mono" style={{ fontSize: '0.65rem', color: 'var(--ink-muted)' }}>
                        Market: {t.market}
                      </span>
                      <span className="font-mono" style={{ fontSize: '0.65rem', color: 'var(--ink-muted)' }}>
                        {t.division}
          ...
          {selectedTeam && (
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              <p className="font-body" style={{ fontSize: '1rem', marginBottom: 16, color: 'var(--ink-soft)' }}>
                ...
                <span className="font-mono" style={{ fontSize: '0.75rem', color: MARKET_TIERS[selectedTier]?.color, marginLeft: 8 }}>
                  Tier {selectedTier} — {MARKET_TIERS[selectedTier]?.label}
                </span>
              </p>
...
function DashboardScreen({ franchise, setFranchise, onSimulate, simulating, seasonRecap, gmGrade, events, onResolveEvent }) {
  ...
  const tabs = ['home', 'roster', 'draft', 'coaching', 'business', 'facilities', 'history'];
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '16px 20px' }}>
      ...
      {tab === 'draft' && <DraftTab franchise={franchise} setFranchise={setFranchise} />}
      {tab === 'coaching' && <CoachingTab franchise={franchise} setFranchise={setFranchise} />}
...
// DRAFT TAB
// ============================================================
function DraftTab({ franchise, setFranchise }) {
  const [prospects, setProspects] = useState(null);
  const [drafted, setDrafted] = useState([]);

  const generateProspects = () => {
    const p = generateDraftProspects(franchise.league, 30, franchise.scoutingStaff);
    setProspects(p);
    setDrafted([]);
  };

  const handleDraft = (prospect) => {
    const player = draftPlayer(prospect, franchise.league);
    setFranchise(prev => ({
      ...prev,
      players: [...prev.players, player],
      totalSalary: Math.round((prev.totalSalary + player.salary) * 10) / 10,
      rosterQuality: Math.round([...prev.players, player].reduce((s, p) => s + p.rating, 0) / (prev.players.length + 1)),
    }));
    setDrafted(prev => [...prev, prospect.id]);
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="card" style={{ padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3 className="font-display section-header" style={{ fontSize: '1rem', marginBottom: 4 }}>Draft Board</h3>
            <p className="font-body" style={{ fontSize: '0.8rem', color: 'var(--ink-muted)' }}>
              Scouting staff level {franchise.scoutingStaff}/3 — higher levels reveal more accurate projections.
            </p>
          </div>
          <button className="btn-primary" onClick={generateProspects}>
            {prospects ? 'New Class' : 'Scout Draft Class'}
          </button>
        </div>
      </div>
      ...
      {drafted.length > 0 && (
        <div className="card" style={{ padding: '10px 16px' }}>
          <span className="stat-label">Drafted this session: </span>
          <span className="font-mono" style={{ fontSize: '0.8rem' }}>{drafted.length} players</span>
        </div>
      )}
      ...
      {prospects && (
        <div className="card" style={{ overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--cream-darker)' }}>
                {['Rank', 'Name', 'Pos', 'Age', 'Projected', 'Upside', 'Trait', ''].map(h => (
                  <th key={h} className="stat-label" style={{ padding: '8px 10px', textAlign: 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {prospects.slice(0, 15).map((p, i) => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--cream-dark)' }}>
                  <td className="font-mono" style={{ padding: '8px 10px', fontWeight: 600 }}>{i + 1}</td>
                  <td className="font-body" style={{ padding: '8px 10px', fontWeight: 500 }}>{p.name}</td>
                  <td className="font-mono" style={{ padding: '8px 10px' }}>{p.position}</td>
                  <td className="font-mono" style={{ padding: '8px 10px' }}>{p.age}</td>
                  <td className="font-mono" style={{ padding: '8px 10px', fontWeight: 600, color: p.projectedRating >= 75 ? 'var(--green)' : 'var(--ink)' }}>
                    {p.projectedRating}
                  </td>
                  <td style={{ padding: '8px 10px' }}>
                    <span className={`badge ${p.upside === 'high' ? 'badge-green' : p.upside === 'mid' ? 'badge-amber' : 'badge-ink'}`}>
                      {p.upside}
                    </span>
                  </td>
                  <td style={{ padding: '8px 10px' }}>
                    {p.trait && <span className="badge badge-ink">{p.trait}</span>}
                  </td>
                  <td style={{ padding: '8px 10px' }}>
                    <button className="btn-secondary" style={{ fontSize: '0.65rem', padding: '4px 10px' }} onClick={() => handleDraft(p)}>
                      Draft
                    </button>
                  </td>
                </tr>
...
// COACHING TAB
// ============================================================
function CoachingTab({ franchise, setFranchise }) {
  ...
  const handleFire = () => {
    const updated = fireCoach(franchise);
  ...
  const handleHire = (candidate) => {
    const updated = hireCoach(franchise, candidate);
    setFranchise(() => updated);
    setCandidates(null);
  };
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card-elevated" style={{ padding: 20 }}>
        <h3 className="font-display section-header" style={{ fontSize: '1rem' }}>Head Coach</h3>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div className="font-display" style={{ fontSize: '1.2rem', fontWeight: 700 }}>{coach.name}</div>
            <div className="font-body" style={{ fontSize: '0.85rem', color: 'var(--ink-soft)', marginTop: 4 }}>
              {coach.personality} — Level {coach.level}/4
            </div>
            <div className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--ink-muted)', marginTop: 4 }}>
              Age {coach.age} — {coach.seasonsWithTeam} seasons with team
            </div>
            <div style={{ display: 'flex', gap: 3, marginTop: 8 }}>
              {[1, 2, 3, 4].map(lvl => (
                <div key={lvl} style={{
                  width: 24, height: 8, borderRadius: 2,
                  background: coach.level >= lvl ? 'var(--red)' : 'var(--cream-darker)',
                }} />
              ))}
            </div>
          </div>
          {!showFire ? (
            <button className="btn-secondary" style={{ borderColor: 'var(--red)', color: 'var(--red)', fontSize: '0.75rem' }}
              onClick={() => setShowFire(true)}>Fire Coach</button>
          ) : (
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn-primary" style={{ fontSize: '0.75rem', padding: '6px 12px' }} onClick={handleFire}>
                Confirm (${coach.level * 2}M buyout)
              </button>
              <button className="btn-secondary" style={{ fontSize: '0.75rem', padding: '6px 12px' }} onClick={() => setShowFire(false)}>
                Cancel
              </button>
            </div>
          )}
      ...
      {candidates && (
        <div className="card" style={{ padding: 20 }}>
          <h3 className="font-display section-header" style={{ fontSize: '1rem' }}>Coaching Candidates</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {candidates.map(c => (
              <div key={c.name} className="card" style={{ padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div className="font-display" style={{ fontSize: '0.95rem', fontWeight: 600 }}>{c.name}</div>
                  <div className="font-body" style={{ fontSize: '0.8rem', color: 'var(--ink-soft)' }}>
                    {c.personality} — Level {c.level}/4 — Age {c.age}
                  </div>
                  <div className="font-body" style={{ fontSize: '0.75rem', color: 'var(--ink-muted)', fontStyle: 'italic', marginTop: 4 }}>
                    {c.backstory}
                  </div>
                </div>
                <button className="btn-primary" style={{ fontSize: '0.7rem', padding: '6px 14px', whiteSpace: 'nowrap' }}
      ...
      {!candidates && coach.name !== 'Interim Coach' && (
        <div className="card" style={{ padding: 16 }}>
          <p className="font-body" style={{ fontSize: '0.85rem', color: 'var(--ink-muted)', textAlign: 'center' }}>
            Your coaching staff is set. Fire your current coach to search for replacements.
          </p>
        </div>
      )}
    </div>
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px' }}>
      ...
      <div className="card" style={{ overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid var(--cream-darker)' }}>
              {['Rank', 'Team', 'Tier', 'W', 'L', 'Win%', 'Fan Rating', 'Market'].map(h => (
                <th key={h} className="stat-label" style={{ padding: '8px 10px', textAlign: 'left' }}>{h}</th>
              ))}
            </tr>
          </thead>
            ...
            {standings.map((t, i) => {
              ...
              return (
                <tr key={t.id} style={{
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <span className="font-mono" style={{ fontSize: '0.65rem', color: getMarketTierInfo(t.market).color }}>
                      T{getMarketTier(t.market)}
                    </span>
                  </td>
...
// ============================================================
// MARKET SCREEN — Stakes & Investments
// ============================================================
function MarketScreen({ leagueTeams, portfolio, stakes, season, setStakes, setPortfolio }) {
  const [offers, setOffers] = useState([]);

  useEffect(() => {
    if (leagueTeams) {
      const o = generateStakeOffers(leagueTeams, portfolio, season);
      setOffers(o);
    }
  }, [leagueTeams, season]);

  const handleBuyStake = (offer) => {
    if (portfolio < offer.price) return;
    setPortfolio(p => Math.round((p - offer.price) * 10) / 10);
    setStakes(prev => [...prev, {
      teamId: offer.teamId,
  ...
  const stakeIncome = calculateStakeIncome(stakes, leagueTeams || { ngl: [], abl: [] });

  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px' }}>
      <h2 className="font-display section-header" style={{ fontSize: '1.3rem' }}>Investment Market</h2>

      {/* Current Stakes */}
      {stakes.length > 0 && (
        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
          <h3 className="font-display" style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 12 }}>Your Stakes</h3>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
            <span className="stat-label">Total Stake Income / Season</span>
            <span className="stat-value" style={{ color: stakeIncome > 0 ? 'var(--green)' : 'var(--red)' }}>${stakeIncome}M</span>
          </div>
          {stakes.map((s, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--cream-dark)' }}>
              <span className="font-body" style={{ fontSize: '0.85rem' }}>{s.teamName}</span>
              <span className="font-mono" style={{ fontSize: '0.8rem' }}>{s.stakePct}% stake</span>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Available Offers */}
      <div className="card" style={{ padding: 20 }}>
        <h3 className="font-display" style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 12 }}>Available Stake Offers</h3>
        {offers.length === 0 ? (
          <p className="font-body" style={{ fontSize: '0.85rem', color: 'var(--ink-muted)' }}>
            {season < 3 ? 'Stake offers become available after Season 3.' : portfolio < 20 ? 'Build your portfolio to $20M+ to unlock stake offers.' : 'No offers available this season. Check back next season.'}
          </p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {offers.map(o => {
              const tierInfo = getMarketTierInfo(o.market);
              return (
                <div key={o.id} className="card" style={{ padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div className="font-display" style={{ fontSize: '0.9rem', fontWeight: 600 }}>{o.teamName}</div>
                    <div className="font-mono" style={{ fontSize: '0.7rem', color: 'var(--ink-muted)' }}>
                      {o.league.toUpperCase()} — {o.record} — Tier {getMarketTier(o.market)} ({tierInfo.label})
                    </div>
                    <div className="font-mono" style={{ fontSize: '0.7rem', color: 'var(--ink-muted)' }}>
                      Valuation: ${o.valuation}M — {o.stakePct}% stake
                    </div>
                  </div>
                  <button
                    className="btn-primary" style={{ fontSize: '0.7rem', padding: '6px 14px' }}
                    disabled={portfolio < o.price}
                    onClick={() => handleBuyStake(o)}
                  >
                    Buy ${o.price}M
                  </button>
                </div>
...
export default function App() {
  ...
  const handleSimulate = async () => {
    ...
    const stakeIncome = calculateStakeIncome(stakes, result.leagueTeams);
    setPortfolio(p => Math.round((p + totalProfit + stakeIncome) * 10) / 10);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, paddingBottom: 40 }}>
        ...
        {screen === 'market' && (
          <MarketScreen
            leagueTeams={leagueTeams} portfolio={portfolio} stakes={stakes}
            season={season} setStakes={setStakes} setPortfolio={setPortfolio}
          />
        )}
        {screen === 'settings' && (
